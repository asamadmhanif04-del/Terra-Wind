...
TH = {
    "dark": {
        "bg": "#080c0a",
        "card_css": "rgba(10,18,14,0.93)",
        "brd_css": "rgba(16,185,129,0.18)",
        "brd2_css": "rgba(255,255,255,0.07)",
        "acc": "#10b981", "acc2": "#059669",
        "gold": "#fbbf24", "suc": "#34d399", "dng": "#f87171",
        "txt": "#d1fae5", "mut": "#6ee7b7",
        "ibg": "#064e3b", "itxt": "#ecfdf5",
        "pbg": "#064e3b", "ptxt": "#ecfdf5",
        "psel": "#022c22", "phov": "#065f46",
        "ebg": "#022c22", "etxt": "#ecfdf5",
        "shd": "0 8px 40px rgba(0,0,0,0.60)",
        "blur": "blur(18px)",
        "m_bg": "#080c0a", "m_card": "#022c22",
        "m_grid": "#064e3b", "m_tick": "#6ee7b7", "m_title": "#10b981",
        "m_poly": "#10b981", "m_pfill": "#065f46",
    },
    "light": {
        "bg": "#f0fdf4",
        "card_css": "rgba(255,255,255,0.96)",
        "brd_css": "rgba(5,150,105,0.20)",
        "brd2_css": "rgba(0,0,0,0.08)",
        "acc": "#059669", "acc2": "#047857",
        "gold": "#d97706", "suc": "#059669", "dng": "#dc2626",
        "txt": "#064e3b", "mut": "#047857",
        "ibg": "#ffffff", "itxt": "#064e3b",
        "pbg": "#ffffff", "ptxt": "#064e3b",
        "psel": "#d1fae5", "phov": "#e6fffa",
        "ebg": "#ecfdf5", "etxt": "#064e3b",
        "shd": "0 4px 22px rgba(5,150,105,0.14)",
        "blur": "blur(10px)",
        "m_bg": "#f0fdf4", "m_card": "#ffffff",
        "m_grid": "#a7f3d0", "m_tick": "#064e3b", "m_title": "#059669",
        "m_poly": "#059669", "m_pfill": "#d1fae5",
    },
}
...
def inject_css():
    ...
    # Generic airport/runway image from Unsplash
    overlay = "rgba(8,12,10,0.88)" if dk else "rgba(240,253,244,0.92)"
    bg_image = "url('https://images.unsplash.com/photo-1542296332-2e4473faf563?q=80&w=2070&auto=format&fit=crop')"
...
[data-testid="stAppViewContainer"]{{
  background-image: linear-gradient({overlay}, {overlay}), {bg_image} !important;
...
.stNumberInput>div>div>input:focus{{
  ...
  box-shadow:0 0 0 3px {'rgba(16,185,129,0.15)' if dk else 'rgba(5,150,105,0.15)'}!important;
...
[data-baseweb="popover"] ul{{
  ...
  border-radius:8px!important;
  box-shadow:0 8px 32px {'rgba(0,0,0,0.55)' if dk else 'rgba(5,150,105,0.18)'}!important;
}}
...
.stDownloadButton>button{{
  ...
  box-shadow:0 3px 14px {'rgba(16,185,129,0.28)' if dk else 'rgba(5,150,105,0.22)'}!important;
...
/* -- FILE UPLOADER FIX FOR DARK SHADE BUG -- */
[data-testid="stFileUploader"] section {{
  background-color: {'rgba(16,185,129,0.08)' if dk else 'rgba(5,150,105,0.05)'} !important;
  border: 2px dashed {'rgba(16,185,129,0.4)' if dk else 'rgba(5,150,105,0.3)'} !important;
  border-radius: var(--rad) !important;
  transition: border .2s !important;
}}
[data-testid="stFileUploader"] section:hover {{
  border-color: {T['acc']} !important;
  background-color: {'rgba(16,185,129,0.12)' if dk else 'rgba(5,150,105,0.08)'} !important;
}}
...
.stSuccess{{background:{'rgba(52,211,153,0.09)' if dk else 'rgba(5,150,105,0.09)'}!important;
  border:1px solid {T['suc']}!important;border-radius:var(--rad)!important;}}
.stError{{background:{'rgba(248,113,113,0.09)' if dk else 'rgba(220,38,38,0.09)'}!important;
  border:1px solid {T['dng']}!important;border-radius:var(--rad)!important;}}
.stWarning{{background:{'rgba(251,191,36,0.09)' if dk else 'rgba(217,119,6,0.09)'}!important;
  border:1px solid {T['gold']}!important;border-radius:var(--rad)!important;}}
.stInfo{{background:{'rgba(16,185,129,0.07)' if dk else 'rgba(5,150,105,0.07)'}!important;
...
/* New Appealing Right-Aligned Theme Toggle Button */
.theme-btn-wrapper {{
  display: flex;
...
.theme-btn-wrapper button {{
  ...
  background: {T['card_css']} !important;
  border: 1.5px solid {T['acc']} !important;
  color: {TXT} !important;
  -webkit-text-fill-color: {TXT} !important;
  box-shadow: 0 4px 15px {'rgba(16,185,129,0.15)' if dk else 'rgba(5,150,105,0.15)'} !important;
...
.theme-btn-wrapper button:hover {{
  ...
  color: #fff !important;
  -webkit-text-fill-color: #fff !important;
  box-shadow: 0 6px 20px {'rgba(16,185,129,0.35)' if dk else 'rgba(5,150,105,0.3)'} !important;
  transform: translateY(-2px) !important;
}}

/* Centered Hero Section */
.ar-hero{{
  position:relative;overflow:hidden; text-align:center; display:flex; flex-direction:column; align-items:center;
  border:1px solid var(--brd);border-radius:20px;
  padding:2.8rem 2.4rem 2.5rem;margin-bottom:1.4rem;
  background:{'linear-gradient(150deg,rgba(8,12,10,0.97),rgba(10,15,12,0.97))' if dk
    else 'linear-gradient(150deg,rgba(230,255,250,0.97),rgba(209,250,229,0.97))'};
...
.ar-hero::after{{
  ...
  background:radial-gradient(circle,
    {'rgba(16,185,129,0.09)' if dk else 'rgba(5,150,105,0.07)'} 0%,transparent 68%);
}}
.ar-eyebrow{{
  ...
  -webkit-text-fill-color:{T['acc']}!important;
  display:flex;align-items:center; justify-content:center; gap:.6rem;margin-bottom:.6rem;
}}
...
.ar-tagline{{
  ...
  max-width:500px;line-height:1.62;margin:0 auto 1.2rem;
}}
.ar-chips{{display:flex;flex-wrap:wrap;justify-content:center;gap:.36rem;}}
.ar-chip{{
  ...
  background:{'rgba(16,185,129,0.08)' if dk else 'rgba(5,150,105,0.07)'};
...
.ar-info-card{{
  background:{'rgba(16,185,129,0.06)' if dk else 'rgba(5,150,105,0.05)'};
...
.ar-type-card{{
  background:{'rgba(16,185,129,0.04)' if dk else 'rgba(5,150,105,0.04)'};
...
.ar-badge-t1{{
  background:{'rgba(16,185,129,0.14)' if dk else 'rgba(5,150,105,0.11)'};
  color:{T['acc']}!important;-webkit-text-fill-color:{T['acc']}!important;
}}
.ar-badge-t2{{
...
.ar-pass{{
  ...
  background:{'rgba(52,211,153,0.12)' if dk else 'rgba(5,150,105,0.12)'};
...
.ar-fail{{
  display:inline-block;
  background:{'rgba(248,113,113,0.12)' if dk else 'rgba(220,38,38,0.12)'};
...
.ar-diag-white{{
  ...
  box-shadow:0 4px 24px {'rgba(0,0,0,0.45)' if dk else 'rgba(5,150,105,0.14)'};
  border:1px solid {'rgba(16,185,129,0.18)' if dk else 'rgba(5,150,105,0.15)'};
}}
...
.ar-tbl th{{
  background:{'#022c22' if dk else '#064e3b'}!important;
  color:#ffffff!important;-webkit-text-fill-color:#ffffff!important;
...
.ar-tbl td.dc{{
  ...
  color:{T['acc']}!important;-webkit-text-fill-color:{T['acc']}!important;
  background:{'rgba(16,185,129,0.04)' if dk else 'rgba(5,150,105,0.04)'}!important;
}}
.ar-tbl tr:nth-child(even) td{{
  background:{'rgba(255,255,255,0.02)' if dk else 'rgba(5,150,105,0.025)'}!important;
}}
.ar-tbl tr.trow td{{
  font-weight:700;
  border-top:2px solid {'rgba(16,185,129,0.3)' if dk else 'rgba(5,150,105,0.3)'};
  background:{'rgba(16,185,129,0.08)' if dk else 'rgba(5,150,105,0.08)'}!important;
  border-bottom:none;
  color:{T['acc']}!important;-webkit-text-fill-color:{T['acc']}!important;
}}
...
.ar-file-banner{{
  background:{'rgba(52,211,153,0.09)' if dk else 'rgba(5,150,105,0.08)'};
...
.gen-wrap .stButton>button{{
  ...
  box-shadow:0 4px 28px {'rgba(16,185,129,0.30)' if dk else 'rgba(5,150,105,0.24)'}!important;
  animation:pulse 2.8s ease-in-out infinite;
}}
.gen-wrap .stButton>button:hover{{
  transform:translateY(-3px)!important;
  box-shadow:0 8px 36px {'rgba(16,185,129,0.50)' if dk else 'rgba(5,150,105,0.40)'}!important;
}}
@keyframes pulse{{
  0%,100%{{box-shadow:0 4px 28px {'rgba(16,185,129,0.26)' if dk else 'rgba(5,150,105,0.20)'};}}
  50%{{box-shadow:0 4px 40px {'rgba(16,185,129,0.52)' if dk else 'rgba(5,150,105,0.42)'};}}
}}
...
.rwy-outer{{
  ...
  background:{'#0a0f0c' if dk else '#d1fae5'};
...
.rwy-fill{{position:absolute;top:0;left:0;bottom:0;
  ...
  background:{'linear-gradient(90deg,rgba(16,185,129,0.07),rgba(16,185,129,0.20))' if dk
    else 'linear-gradient(90deg,rgba(5,150,105,0.08),rgba(5,150,105,0.22))'};
  border-right:2px solid {'rgba(16,185,129,0.60)' if dk else 'rgba(5,150,105,0.55)'};}}
.rwy-plane{{position:absolute;top:50%;transform:translateY(-50%);font-size:22px;
  filter:{'drop-shadow(0 0 6px rgba(16,185,129,0.85))' if dk else 'drop-shadow(0 0 4px rgba(5,150,105,0.55))'};
...
def compass_svg(accent, size=145):
    c = size / 2;
    r = c * 0.9
    d_pts = f"{c},{c - r} {c + r},{c} {c},{c + r} {c - r},{c}"
    d_pts2 = f"{c},{c - r * .75} {c + r * .75},{c} {c},{c + r * .75} {c - r * .75},{c}"
    return (f'<svg width="{size}" height="{size}" viewBox="0 0 {size} {size}"'
            f' xmlns="http://www.w3.org/2000/svg" class="ar-compass">'
            f'<polygon points="{d_pts}" fill="none" stroke="{accent}" stroke-width="1.2" opacity="0.4"/>'
            f'<polygon points="{d_pts2}" fill="none" stroke="{accent}" stroke-width="0.8" opacity="0.2"/>'
            f'<circle cx="{c}" cy="{c}" r="{r * 0.4:.1f}" fill="none" stroke="{accent}" stroke-width="0.5" opacity="0.5"/>'
            f'<polygon points="{c},{c - r * 0.6} {c - 6},{c} {c},{c + r * 0.2} {c + 6},{c}" fill="{accent}" opacity="0.9"/>'
            f'<polygon points="{c},{c + r * 0.6} {c - 6},{c} {c},{c - r * 0.2} {c + 6},{c}" fill="{accent}" opacity="0.4"/>'
            f'<circle cx="{c}" cy="{c}" r="4" fill="{accent}" opacity="0.9"/>'
            f'<circle cx="{c}" cy="{c}" r="2" fill="white" opacity="0.8"/></svg>')
...
def main():
    ...
    # ── THEME TOGGLE ────────────────────────────
    icon = "☀️ Light Theme" if dk else "🌑 Dark Theme"
    togg = st.columns([5, 1])
    with togg[1]:
        st.markdown('<div class="theme-btn-wrapper">', unsafe_allow_html=True)
        if st.button(icon, key="theme_btn", help="Toggle appearance"):
    ...
    steps = [("Upload", "CSV or Excel with wind direction + speed data"),
             ...
             ("Details", "Name, roll no, site — appear in PDF"),
             ("Generate", "Generate your Required Diagrams"),
             ("Download", "PDF Report + Frequency Table CSV export")]
    with hc1:
    ...
    st.markdown("""<div class="ar-type-grid">
      <div class="ar-type-card">
        <div class="ar-type-code">I · S_RUNWAY</div>
        <div class="ar-type-name">Type I — Single Runway</div>
        <span class="ar-type-badge ar-badge-t1">Polygon</span>
      ...
      <div class="ar-type-card">
        <div class="ar-type-code">I · M_RUNWAY</div>
      ...
      <div class="ar-type-card">
        <div class="ar-type-code">II · S_RUNWAY</div>
      ...
      <div class="ar-type-card">
        <div class="ar-type-code">II · M_RUNWAY</div>
    ...
    with ui1:
        stu_name = st.text_input("👤 YOur Name", key="sname")
    with ui2:
        stu_roll = st.text_input("🎓 Roll Number", key="sroll")
    with ui3:
        stu_site = st.text_input("📍 Location", key="ssite")
    with ui4:
        logo_file = st.file_uploader("🖼 Organization Logo",
    ...
    if st.session_state.ready and st.session_state.diagrams:
        ...
        st.info("Thank you! Now you can Download your Report.")
